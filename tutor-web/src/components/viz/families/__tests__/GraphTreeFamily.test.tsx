...
describe("familyRegistry", () => {
  it("registers graph-tree (Plan 3), chart-dist + seq-array + matrix-grid + sort-merge (Plan-V families 4 + 2 + 3 + merge) + class-diagram (Plan-V family 7)", () => {
    // order-insensitive: the registry is a Record, key order is not a contract.
    expect(Object.keys(familyRegistry).sort()).toEqual(["chart-dist", "class-diagram", "graph-tree", "matrix-grid", "seq-array", "sort-merge"]);
  });
});
